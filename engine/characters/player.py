...
class Player(Npc):
    def __init__(self, name='peepeepoopooman', health=10, max_health = 10, alive=True, damage = 5, exp = 0, lvl = 1, defense = 0):
    ...
    def equip_item(self, item):
        if item in inv.player_inventory.items and item.equippable and not item.equipped:
            item.equipped = True
    ...
    def unequip_item(self, item: inv.EquippableItem):
        if item in self.items and item.equippable and item.equipped:
            item.equipped = False
            self.damage -= item.damage
            self.defense -= item.defense
